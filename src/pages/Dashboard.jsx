...
const BorrowerFeed = () => {
  ...
  useEffect(() => {
    const fetchVaults = async () => {
      try {
        const response = await fetch('http://localhost/software/backend/api/vaults/get_available.php');
...
const LenderPortfolio = () => {
  ...
  const handleDeployCapital = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost/software/backend/api/vaults/create.php', {
        method: 'POST',
      ...
      <div className="lg:col-span-1 bg-white p-6 rounded-2xl border border-slate-100 shadow-sm relative overflow-hidden">
        ...
        <form className="space-y-4" onSubmit={handleDeployCapital}>
          <div>
            ...
            <input name="amount" value={formData.amount} onChange={handleChange} type="number" min="1" step="0.01" className="w-full p-3 border-2 border-slate-100 rounded-lg bg-slate-50 focus:bg-white focus:border-rich-gold focus:ring-2 focus:ring-rich-gold/20 outline-none transition-all font-bold text-slate-700" placeholder="e.g. 200" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Interest (%)</label>
              <input name="interest" value={formData.interest} onChange={handleChange} type="number" min="0" max="15" step="0.01" className="w-full p-3 border-2 border-slate-100 rounded-lg bg-slate-50 focus:bg-white focus:border-rich-gold focus:ring-2 focus:ring-rich-gold/20 outline-none transition-all font-bold text-slate-700" placeholder="0 - 15" />
            </div>
            <div>
              <label className="block text-xs font-bold text-slate-500 uppercase tracking-wide mb-1">Days</label>
              <input name="duration" value={formData.duration} onChange={handleChange} type="number" min="1" step="1" className="w-full p-3 border-2 border-slate-100 rounded-lg bg-slate-50 focus:bg-white focus:border-rich-gold focus:ring-2 focus:ring-rich-gold/20 outline-none transition-all font-bold text-slate-700" placeholder="7" />
            </div>
          </div>
          {error && <p className="text-sm font-bold text-red-600">{error}</p>}
          {message && <p className="text-sm font-bold text-green-600">{message}</p>}
          <button disabled={isSubmitting} className="w-full py-3 mt-2 bg-slate-800 text-white font-bold rounded-lg uppercase tracking-wider hover:bg-rich-gold hover:shadow-lg hover:-translate-y-0.5 transition-all disabled:opacity-60 disabled:cursor-not-allowed">
